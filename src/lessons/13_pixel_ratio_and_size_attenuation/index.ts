...
// Because of screen with different pixel ratio, we need to set
// the pixel ratio of the renderer to be the same as the device
// pixel ratio

// We want our particle to have same size on all devices

// We will send pixel ratio as uniform to the shader

// look at firefliesMaterial in the code

// inside vertex shader we will multiply pixel ratio with
// the gl_PointSize

// also don't forget to send mentioned uPixelRatio
// inside on resize event (because we are using devicePixelRatio
// and it will change as we resize the window)

// ----------------- Size Attenuation ---------------------
// (We want particle closer to the camera to be bigger, and ones
// further away to be smaller)
// Inside vertex shader we need to activate size attenuation
// we do this by setting gl_PointSize *= 1.0 / - viewPosition.z
// (we are dividing by z because we are in perspective space)
// (this code is copied from three.js source code about shader chunks)
