...
if (canvas) {
  ...
  const portalMaterial = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uColorStart: { value: new THREE.Color(parameters.uColorStart) },
      uColorEnd: { value: new THREE.Color(parameters.uColorEnd) },
    },
    side: THREE.DoubleSide,
  });

  gui.addColor(parameters, "uColorStart").onChange((color: string) => {
    portalMaterial.uniforms.uColorStart.value.set(color);
  });
  gui.addColor(parameters, "uColorEnd").onChange((color: string) => {
    portalMaterial.uniforms.uColorEnd.value.set(color);
  });
